(() => {
  ...
  function renderProducts() {
    ...
    const newSection = $('#new');
    newSection.hidden = newest.length === 0;
    $$('a[href="#new"]').forEach(link => { link.hidden = newest.length === 0; });
    els.newGrid.innerHTML = newest.map(productCard).join('');
